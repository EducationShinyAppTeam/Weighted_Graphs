    ...
    dashboardHeader(
      ...
      tags$li(
        ...
        tags$a(target = "_blank", 
               icon("comments"),
               href = "https://pennstate.qualtrics.com/jfe/form/SV_7TLIkFtJEJ7fEPz?appName=Weighted_Graphs"
        )
      ),
    ...
    dashboardBody(
      tabItems(
          ...
          fluidRow(
            ...
            sidebarLayout(
              sidebarPanel(
                ...
                matrixInput(inputId = "weights", 
                            value = matrix(rep(c(0,1),3), 
                                           nrow = 6, 
                                           dimnames = list(c("A-B", 
                                                             "A-C", 
                                                             "A-D", 
            ...
            fluidRow(
              ...
              column(width = 6,
                     ...
                     conditionalPanel(
                       ...
                       "Using the time reversibility equations we get that the 
                       long run probability for each state i is \n \\(\\pi_i = 
                       \\frac{\\sum_j \\omega _{ij}}{\\sum_i\\sum_j \\omega_{ij}
                       }\\).\n where \\(\\omega_{ij}\\) is the weight on the 
                       edge connecting nodes \\(i\\) and \\(j\\). You can also 
                       find the long run probabilities using the transition 
                       matrix."
                     ),
                     fluidRow(
                       column(
                         width = 4, 
                         actionButton("checkLR", "Check Answer")),
                       column(
                         width = 8, 
                         uiOutput("correctnessLR"))),
          ...
          tabsetPanel(
            ...
            tabPanel(
              ...
              div(style = "display:inline-block", 
                  p("This problem can be considered as a weighted graph where,")),
              div(style = "display:inline-block", 
                  selectInput(
                    inputId = "fillinAns1", 
                    label = NULL, 
                    choices = c("",
                              ...
                              "potential moves from the knight's position"), 
                    width = "225px")),
              div(style = "display:inline-block", 
              ...
              div(style = "display:inline-block", 
                  numericInput(inputId = "fillinAns3", 
                               label = NULL, 
                               value = NULL, 
                               min = 0, 
                               step = 1, 
              ...
              div(style = "display:inline-block", 
                  selectInput(inputId = "fillinAns4", 
                              label = NULL, 
              ...
              div(style = "display:inline-block", 
                  p(' with the ')),
              div(style = "display:inline-block", 
                  selectInput(inputId = "fillinAns5", 
                              label = NULL, 
                              choices = c("", "vertex", "edge"), 
                              width = "100px")),
              div(style = "display:inline-block", 
            ...
            tabPanel(
              ...
              sidebarLayout(
                sidebarPanel(
                  width = 6,
                  "In modelling this problem, drawing out the entire matrix for 
                  the Markov Chain would be tedious as a chain with 64 states 
                  has 4096 transition probabilities, most of which will be 0.", 
                  "Instead, let's start by figuring out how many branches will 
                  lead from each node (i.e. square on the board). Fill in these 
                  values below.",
                  fluidRow(
                    column(width = 6, 
                           fluidRow(
                             column(width = 8, 
                                    numericInput(inputId = "branchesA", 
                                                 label = "Edges From A", 
                                                 value = 0, 
                                                 min = 0, 
                                                 max = 10)),
                           ...
                           fluidRow(column(width = 8, 
                                           numericInput(inputId = "branchesB", 
                                                        label = "Edges From B",
                                                        value = 0, 
                                                        min = 0,
                                                        max = 10)),
                           ...
                           fluidRow(column(width = 8, 
                                           numericInput(inputId = "branchesC", 
                                                        label = "Edges From C", 
                                                        value = 0, 
                                                        min = 0, 
                                                        max = 10)),
                    ...
                    column(width = 6, 
                           fluidRow(column(width = 8, 
                                           numericInput(inputId = "branchesD", 
                                                        label = "Edges From D", 
                                                        value = 0,
                                                        min = 0, 
                                                        max = 10)),
                           ...
                           fluidRow(column(width = 8, 
                                           numericInput(inputId = "branchesE", 
                                                        label = "Edges From E", 
                                                        value = 0, 
                                                        min = 0, 
                                                        max = 10)),
                ...
                mainPanel(width = 6,
                        ...
                        corresponds to a number of potential moves for the knight.",           
                          dataTableOutput(outputId = "chessBoard", width = 320)
                )
              ),
            ...
            tabPanel(
              ...
              value = "pt2",
              tableOutput("table"),
              "Assume the weight of each edge is 1 since they all must be the 
              same. We will now calculate the long run proportions using the 
              equation: 
              \\(\\pi_i = \\frac{\\sum_j \\omega _{ij}}{\\sum_i\\sum_j \\omega_{ij}}\\)",
              p("Now calculate the sum of all edge weights (the denominator)."),
              fluidRow(
                column(width = 2, 
                       numericInput(inputId = "sumWeights", 
                                    label = "Sum", 
                                    width = '100px', 
                                    value = 0)),
                column(width = 1,  br(),
              ...
              bsButton(inputId = "checkSum", label = "Check Answer"),
              p("Recall that the expected value of the number of jumps it 
                 ...
                 a node of each type?"),
                     ...
                     fluidRow(
                       column(width = 2,
                              numericInput(inputId = "chessAnsA", label = "A", value = 0),
                              uiOutput("ansA")),
                       column(width = 2,
                              numericInput(inputId = "chessAnsB", label = "B", value = 0),
                              uiOutput("ansB")),
                       column(width = 2,
                              numericInput(inputId = "chessAnsC", 
                                           label = "C", 
                                           value = 0),
                              uiOutput("ansC")),
                       column(width = 2,
                              numericInput(inputId = "chessAnsD", 
                                           label = "D", 
                                           value = 0),
                              uiOutput("ansD")),
                       column(width = 2,
                              numericInput(inputId  = "chessAnsE", 
                                           label = "E", 
                                           value = 0),
                              uiOutput("ansE"))
                     ),
                     bsButton(inputId  = "checkFinAns", 
                              label = "Check Answers")
            ),
            tabPanel(
              title = "Simulation",
              ...
              sidebarLayout(
                sidebarPanel( 
                  ...
                  bsButton(inputId = "step", label = "Step"),
                  bsButton(inputId = "resetStep", label = "Reset Step Count"),
                  textOutput("stepCount"),
                  bsButton(inputId = "push", label = "Simulate"),
                  textOutput("text")),
                mainPanel(
                  width = 6,
                  plotOutput(outputId = "ggstyle", click = "start"), # Clickable plot to 
                  # define chess board
        ...
        tabItem(
          ...
          p(
            class = "hangingindent", 
            HTML(paste("Ross S. M. (2014),", 
                       tags$em("Introduction to Probability Models"), 
...
server <- function(input, output, session) {
  ...
  reduceWeights <- reactive({
    ...
    validate(
      need(max(weights())>0 && min(weights() >= 0), 
           paste("All edges must have non-negative weight and at least one edge", 
  ...
  createChainMatrix <- reactive({
    ...
    cMat <- dplyr::filter(reduceWeights(), 
                          to %in% validEdges && from %in% validEdges)
  ...
  output$graph <- renderPlot({plot(x = graph_from_data_frame(d = reduceWeights(), 
                                                       directed = F), 
                                 label = TRUE, 
  ...
  output$uMat <- renderUI({
    matrixInput(inputId = "userMatrix", 
                value = matrix(rep(0, nrow(createChainMatrix())^2), 
  ...
  output$LRuMat <- renderUI({
    matrixInput(inputId = "LRuserMatrix", 
                value = matrix(rep(0, nrow(longRuns())), 
                       nrow = nrow(longRuns()), 
  ...
  checkFillins <- eventReactive(input$submitFillIn, {
    output$fillinFeedback1 <- renderUI({correctnessPic(input$fillinAns1 == 
                                                         "squares on the board")})
    output$fillinFeedback2 <- renderUI({correctnessPic(
    ...
    output$fillinFeedback3 <- renderUI({correctnessPic(!(is.null(input$fillinAns3)) 
                                                       && input$fillinAns3 == 64)})
    output$fillinFeedback4 <- renderUI({
      correctnessPic(input$fillinAns4 == "vertex")})
    output$fillinFeedback5 <- renderUI({
      correctnessPic(input$fillinAns5 == "edge")})})
  ...
  observeEvent(input$next0, {updateTabsetPanel(session = session, 
                                               inputId = "challengeTabs", 
                                               selected = "pt1")})
  observeEvent(input$next1, {updateTabsetPanel(session = session, 
                                               inputId  = "challengeTabs", 
                                               selected = "pt2")})
  
  # Table of numbers of branches and number of occurrences 
  output$table <- renderTable({
    matrix(c(4,2,8,3,20,4,16,6,16,8), 
           nrow = 2, 
  ...
  output$ggstyle <- renderPlot({
    df <- data.frame(x = point$x, y = point$y, 
